...
class RealsensePose:
    def __init__(self, checkpoints_path, w=640, h=480):
        ...
        # self.model = YOLO(r'E:\\my_yolo\\runs\\detect\\train7\\weights\\best.pt')
    ...
    def run(self):
        ...
        while True:
            ...
            if len(boxes.conf) > 0:
                index = np.argmax(boxes.conf.cpu().numpy())
                id = boxes.cls[index]
                ...
                p3 = (xyxy[0], xyxy[3])
                p4 = (xyxy[2], xyxy[3])
                # print(xywhn[0].cpu().numpy())
                # cv2.imwrite('E:\\my_yolo\\data_src\\images\\train\\' + str(13) + '.' + str(count) + '.png', color_image)
                # with open('E:\\my_yolo\\data_src\\labels\\train\\' + str(13) + '.' + str(count) + '.txt', 'w') as f:
                #     f.write(str(13) + ' ' + str(xywhn[0]) + ' ' + str(xywhn[1]) + ' ' + str(xywhn[2]) + ' ' + str(xywhn[3]))
                #     f.close()
            ...
            text = 'FPS: {}, id: {}'.format(int(1 / mean_time * 10) / 10, id)
